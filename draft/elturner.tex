...
This chapter describes two separate algorithms we developed to to perform this task.  Both methods generate watertight 3D meshes that utilize adaptive triangle sizes in order to ensure efficiency of representation.  In Section~\ref{sec:fss_stats}, we discuss probabilistic analysis that is performed on the raw data scans, which can be used to aid the volumetric labeling of the model environment.  Next, in Section~\ref{sec:3dv2013}, we detail our first complex carving technique, which represents the environment volumetrically by an advancing voxel boundary.  We refer to this technique as ``voxel carving'', whcih produces a piece-wise planar, watertight model of the interior environment of the building~\cite{Turner13}.  In Section~\ref{sec:procarve}, we introduce a new method that expands on the voxel carving method by introducing the probablistic models discussed in Section~\ref{sec:fss_stats}.  This method also performs the volumetric labeling using an octree structure rather than voxels, to allow for adaptive resolutions within the model and to allow efficient parallel processing.  Again, the method results in a watertight mesh of the environment that can be represented via piece-wise planar surfaces.  Lastly, in Section~\ref{sec:carving_results}, we show comparitive results of these two methods on real-world data.

% section on probability analysis on input data
\section{Statistical Models of Input Scans}
\label{sec:fss_stats}

% TODO section on fss statistics, including lots of expansion on
%	noisypath
%	sensor noise model
...
% TODO incorporate siggraph paper here

% results section for 3D carving
\section{Results}
\label{sec:carving_results}

% TODO results figures of 3d carving from both 3dv2013 and siggraph
...
\begin{figure}
	...
	\caption{A visual comparison of meshing method for a hallway model:  (a) photograph of scanned area; (b) an existing voxel carving method~\cite{Carving}; (c) the proposed method at 5 cm resolution.}
...
% TODO copy 'main idea' section from siggraph paper
% TODO modify the 'overview' subsection's pipeline figure
